...
async function call(path, options = {}) {
  try {
    ...
    if (res.status === 401) {
      // The session expired or was signed out elsewhere. Nothing observes the
      // cookie (it is httpOnly), so announce it and let the panel drop back to
      // the login form instead of showing "Not signed in" on every action.
